...
final class PublicInvitationCollectionController extends AbstractController
{
    #[Route('/api/v1/public-invitations', name: 'api_core_public_invitations_collection', methods: ['GET', 'POST'])]
    public function __invoke(
        ...
        if ($request->isMethod('GET')) {
            ...
            $data = array_map(static fn (PublicInvitation $invitation): array => [
        ...
        $invitation = (new PublicInvitation())
            ...
            ->setCampaignType((string) ($payload['campaign_type'] ?? 'marketing'))
            ->setMessageSubject((string) ($payload['message_subject'] ?? 'Explora Mi Monchis MX antes que nadie'))
        ...
        return $this->json([
            'data' => [
                ...
                'status' => $invitation->getStatus(),
                'expires_at' => $invitation->getExpiresAt()->format(DATE_ATOM),
                'invitation_url' => sprintf('/alpha/access?token=%s', $plainToken),
            ],
            'meta' => [],
            'errors' => [],
        ], 201);
